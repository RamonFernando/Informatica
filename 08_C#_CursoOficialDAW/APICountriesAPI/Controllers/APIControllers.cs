    ...
    internal class APIControllers
    {
        ...
        public static List<FavoriteItemList> AddToFavoriteList(string stringJson)
        {
            ...
            FavoriteItemList atributes = new FavoriteItemList()
            {
                ...
                Common = (string)character["name"]["common"],
                Official = (string)character["name"]["official"],

                // * Cambiar este atributo si hay una clase objeto con parametros dentro Ej. Name: {common: Spain, official: Spain}
                Name = new ClassName
                {
                    Common = (string)character["name"]["common"],
                    Official = (string)character["name"]["official"]
        ...
        public static void ShowFavoriteList()
        {
            ...
            foreach (var item in favoriteList)
            {
                Console.WriteLine($"\n{count++}ºFavorite:" +
                    $"\n-------------------------" +
                    $"\nNombre oficial: {(item.Name.Official == null ? "Unknown" : item.Name.Official)}" +
                    $"\nNombre comun: {(item.Name.Common == null ? "Unknown" : item.Name.Common)}"                   
                    );
            }
            PrintWaitForPressKey();
        } // ShowFavoriteList
