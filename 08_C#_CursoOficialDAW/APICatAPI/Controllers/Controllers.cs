using Newtonsoft.Json;
using Newtonsoft.Json.Linq;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Net.Http;
using System.Threading.Tasks;
using static APICatAPI.Program;
using static APICatAPI.Models;

// Metodos
// 1. (26)  SearchByName()          : Buscar por Nombre
// 2. (63)  AddToFavoriteList()     : Agrega a la lista de favoritos
// 3. (103)  ShowFavoriteList()      : Mostrar la lista de favoritos
// 4. (129) RemoveFavoriteList()    : Elimina de la lista de favoritos
// 5. (150) ValidateInput()         : Valida el input
// 6. (162) PrintWaitForPressKey()  : Tocar para continuar
// 7. (169) HandlerException()      : Manejo de EXCEPCIONES

namespace APICatAPI
{
    internal class Controllers
    {
        // Metodos
        // 1. BUSCAR por Nombre
        public static async Task<JArray> SearchByName(string name)
        {
            var Url = $"https://api.thecatapi.com/v1/breeds/search?q={name}";
            var response = await client.GetAsync(Url);

            // Validamos la respuesta del servidor
            if (!response.IsSuccessStatusCode)
            {
                if (response.StatusCode == System.Net.HttpStatusCode.BadRequest)
                {
                    Console.WriteLine("Gato no encontrado");
                    PrintWaitForPressKey();
                    return null;
                } 

                Console.WriteLine($"Error: Codigo: {response.StatusCode}\n Info: {response.ReasonPhrase}");
                PrintWaitForPressKey();
                return null;
            }

            string stringJson = await response.Content.ReadAsStringAsync();

            // Valida que el Json no empiece con un [corchete] (demuestra que no se ha encontrado el digimon)
            if (!stringJson.TrimStart().StartsWith("["))
            {
                Console.WriteLine("Gato no encontrado");
                PrintWaitForPressKey();
                return null;
            }

            // Conviertimos el JSON en un array y luego creamos un nuevo array para almacenar los digimons
            JArray arrayJson = JArray.Parse(stringJson);

            return arrayJson;
        } // SearchByName

        // 2. AGREGAR a la lista de favoritos
        public static List<FavoriteItemList> AddToFavoriteList(string stringJson)
        {
            Console.WriteLine("Desea agregar el Gato a la lista de favoritos S/N?");
            string inputAnswer = Console.ReadLine();

            if (inputAnswer != "S" && inputAnswer != "s")
            {
                Console.WriteLine("Gato no agregado a la lista de favoritos");
                PrintWaitForPressKey();
                return favoriteList;

            }
            // Convierte el JSON en un array
            var array = JArray.Parse(stringJson); // Error examen

            // Convierte el primer elemento del array en un objeto FavoriteItemList
            var cat = array[0];

            FavoriteItemList atributes = new FavoriteItemList()
            {   
                Id = (string)cat["id"],
                Name = (string)cat["name"],
                Origin = (string)cat["origin"],
                Description = (string)cat["description"],
            };

            // Comprobamos si el digimon ya esta en la lista
            if (favoriteList.Any(d => d.Name == atributes.Name))
            {
                Console.WriteLine($"El gato {atributes.Name} ya esta en la lista de favoritos");
                PrintWaitForPressKey();
                return favoriteList;
            }
            favoriteList.Add(atributes);
            Console.WriteLine("Gato agregado a la lista de favoritos");
            PrintWaitForPressKey();
            return favoriteList;
        } // AddToFavoriteList

        // 3. MOSTRAR la lista de favoritos
        public static void ShowFavoriteList()
        {
            Console.WriteLine("Mostrando lista de favoritos");
            Console.WriteLine("====================================");
            if (favoriteList.Count == 0)
            {
                Console.WriteLine("La lista de favoritos esta vacia");
                PrintWaitForPressKey();
                return;
            }
            int count = 1;

            // *poner atributos de lista de favoritos (FavoriteItemList)*
            foreach (var item in favoriteList)
            {
                Console.WriteLine($"\n{count++}ºFavorite:" +
                    $"\n-------------------------" +
                    $"\nId: {item.Id}" +
                    $"\nName: {item.Name}" +
                    $"\nOrigin: {(item.Origin == null ? "Unknown" : item.Origin)}" +
                    $"\nDescription: {(item.Description == null ? "Unknown" : item.Description)}");
            }
            PrintWaitForPressKey();
        } // ShowFavoriteList

        // 4. BORRAR digimon de la lista de favoritos
        public static void RemoveFavoriteList()
        {
            Console.Write("Ingrese el id de la Lista de favoritos que desea borrar: ");
            int index = ValidateInput(Console.ReadLine()) - 1;

            if (index < 0 || index >= favoriteList.Count)
            { // Comprobamos que el index sea valido
                Console.WriteLine("\nEl id ingresado no se encontro el la lista de favoritos o no es valido");
                PrintWaitForPressKey();
                return;
            }
            Console.Write($"\nDesea borrar el Gato {favoriteList[index].Name} de la lista de favoritos S/N?");
            string inputResp = Console.ReadLine();
            if (inputResp != "S" && inputResp != "s") return;

            favoriteList.RemoveAt(index);
            Console.WriteLine("\nGato borrado de la lista de favoritos");
            PrintWaitForPressKey();
        } // RemoveFavoriteList

        // 5. VALIDAR Input
        public static int ValidateInput(string input)
        {
            if (!int.TryParse(input, out int num))
            {
                // Console.WriteLine("Opcion no valida");
                //PrintWaitForPressKey();
                return -1;
            }
            return num;
        }

        // 6. PRINT (esperar tecla)
        public static void PrintWaitForPressKey()
        {
            Console.WriteLine("\nPresiona cualquier tecla para continuar...");
            Console.ReadKey();
        }

        // 7. Manejo de EXCEPCIONES
        public static void HandlerException(Exception ex)
        {
            if (ex is HttpRequestException HttpEx)
            {
                Console.WriteLine($"Error de Peticion HTTP: {HttpEx.Message}\n Info: {HttpEx.HResult}");
                return;
            }
            if (ex is JsonException JsonEx)
            {
                Console.WriteLine($"Error de Lectura JSON: {JsonEx.Message}\n Info: {JsonEx.HResult}");
                return;
            }
            Console.WriteLine($"Error desconocido: {ex.Message}\n Info: {ex.HResult}");
        }

    } // controllers
}
