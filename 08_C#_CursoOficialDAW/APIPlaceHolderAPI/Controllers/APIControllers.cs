...
using static APIPlaceHolderAPI.APISaveJson;
...
namespace APIPlaceHolderAPI
{
    ...
    {
        // 1. BUSCAR por nombre
        public static async Task<JArray> SearchByUserId(int id) // *si es objeto cambiar de JArray a JObject*
        {
            var Url = $"https://jsonplaceholder.typicode.com/todos?id={id}"; // *cambiar url*
            ...
            JArray arrayJson = JArray.Parse(stringJson); // *cambiar de JArray a JObject*
        ...
        public static List<FavoriteItemList> AddToFavoriteList(string stringJson)
        {
            ...
            FavoriteItemList atributes = new FavoriteItemList()
            {
                ...
                Completed = (bool)character["completed"]
            };

            // Comprobamos si la tarea ya esta en la lista
            if (favoriteList.Any(d => (d.UserId == atributes.UserId) && (d.Id == atributes.Id) && (d.Title == atributes.Title) && (d.Completed == atributes.Completed)))
            {
                Console.WriteLine($"La {NameProperty} \"{atributes.Title}\" ya esta en la lista de favoritos");
                PrintWaitForPressKey();
                return favoriteList;
            }
        ...
        public static void ShowFavoriteList()
        {
            ...
            foreach (var item in favoriteList)
            {
                Console.WriteLine($"\n{count++}ºFavorite:" +
                    $"\n-------------------------" +
                    $"\nUsuario: {(item.UserId == 0 ? "Unknown" : item.UserId.ToString())}" +
                    $"\nId: {(item.Id == 0 ? "Unknown" : item.Id.ToString())}" +
                    $"\nTitulo: {item.Title}" +
        ...
        public static void RemoveFavoriteList()
        {
            ...
            Console.Write($"\nDesea borrar la {NameProperty} \"{favoriteList[index].Title}\" de la lista de favoritos S/N?");
            string inputResp = Console.ReadLine();
            if (inputResp != "S" && inputResp != "s") return;

            favoriteList.RemoveAt(index);
            Console.WriteLine($"\n{NameProperty} borrada de la lista de favoritos");
            PrintWaitForPressKey();
