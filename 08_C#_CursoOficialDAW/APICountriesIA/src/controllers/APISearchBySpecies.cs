namespace APICountriesIA
{
    public class APISearchBySpecies
    {
        /*public static async Task SearchBySpecies()
        {
            ...
            Console.Write(
                string.Format(readOnlyMessage, NAME_TYPE_PROP_ESPECIE, NAME_PROP) +
                "(Human, Alien, Robot): ");
            string? inputSpecies = Console.ReadLine();

            ValidateInputString(inputSpecies); // null o "" = Entrada no valida

            var json = await GetItemApiAsync($"{BASE_URL_CHARACTERS}/?species={inputSpecies}"); // no null
            
            if(!isValidateJsonNotNull(json, NAME_TYPE_PROP_ESPECIE, inputSpecies)) return;
            JsonDocument jsonNotNull = json!; // ya validado no null
            
            List<JsonElement> results = ExtractResults(jsonNotNull);
            if(!isValidateResults(results, NAME_TYPE_PROP_ESPECIE, inputSpecies)) return; // count 0 = no encontrado
